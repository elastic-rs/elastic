...
pub fn post_index_type<'a>(client: &'a mut Client,
                           ...
                           body: String)
                           -> Result<Response> {
    let mut url_fmtd = String::with_capacity(base.len() + 1 + 1 + 11 + index.len() + _type.len());
...
pub fn post_index_type_id<'a>(client: &'a mut Client,
                              ...
                              body: String)
                              -> Result<Response> {
    let mut url_fmtd = String::with_capacity(base.len() + 1 + 1 + 1 + 11 + index.len() +
                                             _type.len() +
                                             id.len());
    url_fmtd.push_str(base);
    url_fmtd.push_str("/");
...
pub fn get_index_type_id<'a>(client: &'a mut Client,
                             base: &'a str,
                             index: &'a str,
                             _type: &'a str,
                             id: &'a str)
