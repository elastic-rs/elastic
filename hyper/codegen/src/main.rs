...
fn gen_from_source(source_dir: &str, dest_dir: &str) -> Result<(), String> {
	...
	for endpoint in parsed {
		...
		if is_new {
			...
			try!(emitter.emit(&quote_stmt!(&mut cx, use hyper::client::Client;), &cx, &mut src_file).map_err(|e| e.description().to_string()));
			try!(emitter.emit_str(&"\n", &mut src_file).map_err(|e| e.description().to_string()));

			try!(emitter.emit(&quote_stmt!(&mut cx,
				#[allow(unused_imports)]
				use hyper::client::Body;
			), &cx, &mut src_file).map_err(|e| e.description().to_string()));
			try!(emitter.emit_str(&"\n", &mut src_file).map_err(|e| e.description().to_string()));

			try!(emitter.emit(&quote_stmt!(&mut cx, use hyper::client::response::Response;), &cx, &mut src_file).map_err(|e| e.description().to_string()));
			try!(emitter.emit_str(&"\n", &mut src_file).map_err(|e| e.description().to_string()));

			try!(emitter.emit(&quote_stmt!(&mut cx, use hyper::error::Result;), &cx, &mut src_file).map_err(|e| e.description().to_string()));
			try!(emitter.emit_str(&"\n\n", &mut src_file).map_err(|e| e.description().to_string()));

			try!(emitter.emit(&quote_stmt!(&mut cx, use ::RequestParams;), &cx, &mut src_file).map_err(|e| e.description().to_string()));
			try!(emitter.emit_str(&"\n\n", &mut src_file).map_err(|e| e.description().to_string()));
		...
		for (_, fun) in fun_sigs_distinct {
			...
			try!(emitter.emit(&rs_fun, &cx, &mut src_file).map_err(|e| e.description().to_string()));
			try!(emitter.emit_str(&"\n", &mut src_file).map_err(|e| e.description().to_string()));
			try!(src_file.sync_all().map_err(|e| e.description().to_string()));
		}
