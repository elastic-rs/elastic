...
pub fn derive_elastic_type(input: proc_macro::TokenStream) -> TokenStream {
    let mut expanded = quote::Tokens::new();
    let ast = syn::parse_macro_input(&input.to_string()).unwrap();

    let genned = elastic_type::expand_derive(quote!(::elastic_types), &ast);
...
#[proc_macro_derive(DateFormat, attributes(elastic))]
pub fn derive_date_format(input: proc_macro::TokenStream) -> TokenStream {
    let mut expanded = quote::Tokens::new();
    let ast = syn::parse_macro_input(&input.to_string()).unwrap();

    let genned = date_format::expand_derive(quote!(::elastic_types), &ast);
