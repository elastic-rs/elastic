...
fn can_emit_rs_fn_to_file() {
	...
	let result = emitter.emit(&fun, &cx, &mut f);
...
fn can_emit_rs_fn_with_fmt_body_to_file() {
	...
	.set_return::<String>()
	.add_body_stmts(vec![url_stmt, quote_stmt!(&mut cx, $url_ident).unwrap()]);
...
fn can_emit_rs_fn_with_push_body_to_file() {
	...
	let (url_ident, mut url_stmts) = url_push_decl(base, parts.iter().map(|p| p.as_str()), params.to_vec());
	url_stmts.push(quote_stmt!(&mut cx, $url_ident).unwrap());
