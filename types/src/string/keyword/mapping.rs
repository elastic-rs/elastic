...
Self: ElasticFieldMapping<()> + Sized + Serialize {
	...
	/// impl ElasticKeywordMapping for MyStringMapping {
	/// 	//Overload the mapping functions here
	/// 	fn fields() -> Option<BTreeMap<&'static str, ElasticKeywordField>> {
	///			let mut fields = BTreeMap::new();
	///
	/// 		//Add another string type as a sub field
	/// 		fields.insert("raw", ElasticKeywordField::String(
	...
	/// 		fields.insert("count", ElasticKeywordField::TokenCount(
	/// 			ElasticTokenCountFieldMapping::default())
	/// 		);
	///
	/// 		//Add a `completion` suggester as a sub field
	/// 		fields.insert("comp", ElasticKeywordField::Completion(
